...
def load_dictionary(infile):
    ...
    #ging through loop to split and save each data to the dictionary
    for i in line:
        i = i.strip()
        Data1 = i.split(", ")
...
def is_present(name,date,data):
    ...
    #condititonal statement to find the person is present or not
    if name not in data:
        return False
    else:
...
#created a new empty list
    ...
    for element in data:
        ...
        for index in data[element]:
            ...
            mins = int(date2[1])
            secs = int(date2[2])
            #Converting the hour, mins to seconds indsde the loop
            hour_sec = hour * seconds_square
            ...
            #Condtional statement to check whether the date is equal to the date given in the parameter
            if date1 == date:
...
def get_first_student_to_enter(date,data):
    ...
    #goung through the loop to and spliting the date from the values
    for element in data:
        for index in data[element]:
            list = index.split(", ")
            date1 = list[1]
    ...
    for i in checked_list:
        ...
        if (time < earliest_time):
            ...
            earliest_name = i[0]
    #Retruning the name (prints the name who attended the class first)
...
if __name__ == '__main__':
    ...
    text =
    rosters = load_roster("rosters.txt")
